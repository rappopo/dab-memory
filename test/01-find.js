...
describe('find', function () {
  ...
  */
  ...
  it('should return filtered values', function () {
    const cls = new Cls(lib.options)
    return expect(cls.find({ query: { _id: 'jack-bauer' }})).to.eventually.have.property('data').that.have.lengthOf(1).and.containSubset([lib.options.data[0]])
  })
  ...
  it('should sort in descending order', function () {
    const cls = new Cls(lib.options)
    return expect(cls.find({ sort: [{ _id: 'desc' }] })).to.eventually.have.property('data').that.containSubset([lib.options.data[1]])
  })
  */
})
